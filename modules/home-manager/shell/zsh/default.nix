...
in with lib; {
  ...
  config = mkIf cfg.enable {
    programs.zsh = {
      ...
      shellAliases = {
        urldecode =
          "${pkgs.python3}/bin/python3 -c 'import sys, urlparse as ul; print(ul.unquote_plus(sys.stdin.read()))'";

        urlencode =
          "${pkgs.python3}/bin/python3 -c 'import sys, urlparse as ul; print(ul.quote_plus(sys.stdin.read()))'";
